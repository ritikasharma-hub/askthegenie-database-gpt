...
# Try to find .env file by traversing up directories
...
<head>
    ...
    <title>AskTheGenie Database AI Chat</title>
...
<body>
    <div class="max-w-6xl mx-auto p-5">
        ...
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-vanna-navy mb-2 font-serif">AskTheGenie Database AI Chat</h1>
            <p class="text-lg font-mono font-bold text-vanna-teal mb-4">DATABASE AI CHAT</p>
            <p class="text-slate-600 mb-4">Interactive AI Assistant powered by AskTheGenie</p>
        ...
        <div id="loginContainer" class="max-w-md mx-auto mb-10 bg-white p-8 rounded-xl shadow-lg border border-vanna-teal/30">
            <div class="text-center mb-6">
                ...
                <p class="text-sm text-slate-600">Enter your mail and Password to access the chat</p>
            </div>

            <div class="mb-4">
                <label for="idInput" class="block mb-2 text-sm font-medium text-vanna-navy">Email ID</label>
                <input 
                    type="text" 
                    id="idInput" 
        ...
        <div id="chatSections" class="hidden">
            <div class="bg-white rounded-xl shadow-lg h-[600px] overflow-hidden border border-vanna-teal/30">
                <vanna-chat
                    title="AskTheGenie Database AI Chat"
    ...
    // Injected from .env
    const RAW_CREDS = {json.dumps(email_credentials)};

    const CREDS = Object.fromEntries(
        RAW_CREDS.split(",").map(pair => pair.split(":"))
    );
        // Cookie helpers
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            ...
            loginButton.addEventListener('click', () => {{
                ...
                // Demo validation - for this specific request we are just checking availability
                // But let's add a basic check to "simulate" security if desired, 
                // or just allow any non-empty credential for the demo as before.
                // The prompt says "Only verified team members... should be allowed".
                // Since I can't change the backend, I'll hardcode a check here.
                // Or I can just trust the input for the "demo" persistence.
                
                // Let's implement a simple hardcoded check for demonstration
                // User: admin, Pass: password123
                // Or allow any for now since the prompt says "Modify the login page functionality... implement a standard ID and Password login field"
                
                // 1️⃣ Check email exists in .env
                if (!CREDS[userId]) {{
                ...
                if (CREDS[userId] !== password) {{
                    alert("Incorrect password");
                    return;
                }}

                // ✅ Success
                setCookie('vanna_email', userId);
                loginContainer.classList.add('hidden');
                loggedInStatus.classList.remove('hidden');
                chatSections.classList.remove('hidden');
                loggedInEmail.textContent = userId;
            }});
                ...
                if (!customElements.get('vanna-chat')) {{
                    document.querySelector('vanna-chat').innerHTML = `
                        <div class="p-10 text-center text-gray-600">
                            <h3 class="text-xl font-semibold mb-2">AskTheGenie Chat Component</h3>
